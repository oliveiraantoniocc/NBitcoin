		...
		* rest is 0.*/
		...
		public void test_ecdsa_edge_cases()
		{
			...
			for (i = 0; i < 1; i++)
			{
				...
				var key3 = ctx.CreateECPrivKey(new byte[]{
					0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
					0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
					0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
					0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01,
				...
				Assert.False(key3.TrySignECDSA(msg2, new PrecomputedNonceFunction(nonce), out sig));
				Assert.False(key3.TrySignECDSA(msg2, new PrecomputedNonceFunction(nonce2), out sig));
				msg2[31] = 0xaa;
				Assert.True(key3.TrySignECDSA(msg2, new PrecomputedNonceFunction(nonce), out sig));
				...
				Assert.False(key3.TrySignECDSA(new ReadOnlySpan<byte>(), new PrecomputedNonceFunction(nonce2), out sig));
				//Assert.True(ecount == 2);
				var emptykey = key3.Clone();
				emptykey.Clear();

				Assert.Throws<ObjectDisposedException>(() => emptykey.TrySignECDSA(msg2, new PrecomputedNonceFunction(nonce2), out sig));
				//Assert.True(ecount == 3);
				Assert.True(key3.TrySignECDSA(msg2, new PrecomputedNonceFunction(nonce2), out sig));
				pubkey = key3.CreatePubKey();
				Assert.False(pubkey.SigVerify(null as SecpECDSASignature, msg2));
				// CHECK(ecount == 4);
				Assert.False(pubkey.SigVerify(sig, new ReadOnlySpan<byte>()));
				// CHECK(ecount == 5);
				//Assert.False(NULL.SigVerify(sig, msg2));
				// CHECK(ecount == 6);
				Assert.True(pubkey.SigVerify(sig, msg2));
				// CHECK(ecount == 6);
			...
			// stackalloc should stay outside loops to avoid potential stack overflows
			Span<Scalar> sr2 = stackalloc Scalar[512];
			...
			for (t = 0; t < 2; t++)
			{
				...
				key4.AsSpan().Fill(0xff);
				// With NBitcoin, we can't even create an incorrect ECPrivKey
				Assert.False(ctx.TryCreateECPrivKey(key4, out _));
				//CHECK(secp256k1_ecdsa_sign(ctx, &sig, msg4, key4, NULL, extra) == 0);
				...
				key4[31] = 1;
				var keyo = ctx.CreateECPrivKey(key4);
				Assert.False(keyo.TrySignECDSA(msg4, new RFC6979TestFailNonceFunction(extra), out sig));
				Assert.Null(sig);
				/* The retry loop successfully makes its way to the first good value. */
				Assert.True(keyo.TrySignECDSA(msg4, new RFC6979TestRetryNonceFunction(extra), out sig));
				Assert.NotNull(sig);
				Assert.True(keyo.TrySignECDSA(msg4, new RFC6979NonceFunction(extra), out sig2));
				Assert.NotNull(sig2);
